...
export function FieldSettings( { field, onChange } ) {
	...
	return (
		<section
			className="corex-flow-editor__field-settings"
			aria-labelledby="corex-field-settings-title"
		>
			<h3 id="corex-field-settings-title">
				{ __( 'Field settings', 'corex' ) }
			</h3>
			<div className="corex-flow-editor__form-grid">
				...
				/>
				<TextField
					label={ __( 'Placeholder', 'corex' ) }
					name="placeholder"
					value={ field.placeholder }
					onChange={ change }
				/>
				<TextField
					label={ __( 'Help text', 'corex' ) }
					name="help_text"
					value={ field.help_text }
					onChange={ change }
				/>
				<TextField
					label={ __( 'Default value', 'corex' ) }
					name="default_value"
				...
				<div className="corex-field">
					...
					<CorexSelect
						id="corex-flow-field-width"
						label={ __( 'Width', 'corex' ) }
						value={ field.width || 'full' }
						options={ FIELD_WIDTHS }
						onChange={ ( value ) =>
							change( { target: { name: 'width', value } } )
						}
						block
					/>
				</div>
				<div className="corex-field">
					...
					<CorexSelect
						id="corex-flow-field-personal-data"
						label={ __( 'Personal data class', 'corex' ) }
						value={ field.personal_data_class || 'none' }
						options={ PERSONAL_DATA_CLASSES }
						onChange={ ( value ) =>
							change( {
								target: { name: 'personal_data_class', value },
							} )
						}
						block
					/>
				...
				<label
					className="corex-flow-editor__check"
					htmlFor="corex-flow-field-required"
				>
					<input
						id="corex-flow-field-required"
						type="checkbox"
						name="required"
