...
export default function ExportDialog( { source, state, close, create } ) {
	...
	const [ scope, setScope ] = useState(
		state.selected.length ? 'selected' : 'filtered'
	);
	...
	const submit = async () => {
		const exportRun = await create( {
			scope,
			selected_ids: scope === 'selected' ? state.selected : [],
			query: scope === 'filtered' ? state.query : {},
			columns,
			format,
	...
	return (
		...
		>
			...
			<CorexSelect
				...
				options={ [
					...
					...( state.selected.length
						...
						: [] ),
					{
						label: __( 'All accessible rows', 'corex' ),
						value: 'all',
					},
				] }
			/>
			<fieldset className="corex-data__column-picker">
			...
			{ personal && (
				<CheckboxControl
					label={ __(
						'I understand this export contains personal data and must be handled securely.',
						'corex'
					) }
			...
			<div className="corex-data__dialog-actions">
				...
				<Button
					...
					onClick={ submit }
					disabled={
						columns.length === 0 || ( personal && ! acknowledged )
					}
				>
