...
jest.mock( '@wordpress/server-side-render', () => () => null, {
	virtual: true,
} );
jest.mock(
	'@wordpress/element',
...
jest.mock(
	'@wordpress/api-fetch',
	() => jest.fn( () => Promise.resolve( { data: { flows: [] } } ) ),
	{ virtual: true }
);
...
import {
	flowOptions,
	normalizePublishedFlows,
	registerFlowBlock,
} from '../flowBlockEditor.js';
...
describe( 'persisted flow blocks', () => {
	...
	test( 'normalizes only published flow records from the REST envelope', () => {
		...
		expect( flowOptions( [ { id: 2, name: 'Live' } ] )[ 1 ] ).toEqual( {
			label: 'Live',
			value: '2',
		} );
	} );
	...
	test.each( [
		...
		'registers %s as a dynamic persisted-flow block',
		( metadata, variant ) => {
			...
			expect( apiFetch ).toHaveBeenCalledWith( {
				path: '/corex/v1/flows?state=published',
			} );
		}
	);
} );
