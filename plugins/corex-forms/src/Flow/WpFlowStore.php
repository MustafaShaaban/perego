...
/**
 * Private WordPress persistence for flows and immutable versions.
 */
final class WpFlowStore implements FlowStore
{
    public const POST_TYPE = 'corex_flow_record';

    private const META_TYPE = '_corex_flow_type';
    private const META_SLUG = '_corex_flow_slug';
    private const META_PAYLOAD = '_corex_flow_payload';
    private const MAX_RECORDS = 500;

    public function registerPostType(): void
    {
        register_post_type(self::POST_TYPE, [
            'label' => __('CoreX flows', 'corex'),
            'public' => false,
            'show_ui' => false,
            'supports' => ['title'],
    ...
    public function update(int $id, string $name, array $payload): bool
    {
        ...
        $updated = wp_update_post(['ID' => $id, 'post_title' => $name], true);
    ...
    {
        $this->assertKey($type);
        $metaQuery = [['key' => self::META_TYPE, 'value' => $type, 'compare' => '=']];
        if ($slug !== null) {
            $this->assertSlug($slug);
            $metaQuery[] = ['key' => self::META_SLUG, 'value' => $slug, 'compare' => '='];
        }
        ...
        return array_map($this->record(...), (new WP_Query($arguments))->posts);
    }
    ...
    private function assertKey(string $value): void
    {
        if (preg_match('/^[a-z][a-z0-9_]*$/', $value) !== 1) {
            throw new InvalidArgumentException(__('Flow record type is invalid.', 'corex'));
        }
    }
    ...
    private function assertSlug(string $slug): void
    {
        if ($slug === '' || strlen($slug) > 191 || preg_match('/^[a-z0-9][a-z0-9_.-]*$/i', $slug) !== 1) {
            throw new InvalidArgumentException(__('Flow slug is invalid.', 'corex'));
        }
    }
}
