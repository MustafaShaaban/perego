	...
	for ( const [ index, raw ] of lines.entries() ) {
		...
		if ( ! match ) {
			errors.push(
				`${ file }:${
					index + 1
				}: unsupported or malformed TOML assignment`
			);
			continue;
		}
...
export function parseClaudeAgent( text, file = '<agent>' ) {
	...
	if ( ! match ) {
		throw new Error(
			`${ file }: YAML frontmatter must be delimited by ---`
		);
	}
	...
	for ( const [ index, line ] of match[ 1 ].split( /\r?\n/ ).entries() ) {
		...
		if ( ! pair ) {
			throw new Error(
				`${ file }:${ index + 2 }: malformed YAML frontmatter field`
			);
		}
...
function assertValue( values, key, expected, file, errors ) {
	if ( values.get( key ) !== expected ) {
		errors.push(
			`${ file }: expected ${ key } = ${ JSON.stringify( expected ) }`
		);
	}
}
...
function scanHygiene( root, files, errors ) {
	...
	for ( const file of files ) {
		...
		for ( const pattern of forbidden ) {
			if ( pattern.test( text ) ) {
				errors.push(
					`${ file }: contains forbidden deprecated model, credential-like value, or absolute local path`
				);
			}
		}
	}
	for ( const file of files ) {
		...
		if ( isAbsolute( path ) || path.startsWith( '..' ) ) {
			errors.push( `${ file }: escapes validation root` );
		}
	}
}
...
export function validateRepository( rootInput ) {
	...
	for ( const file of codexFiles ) {
		...
		for ( const key of [
			...
			if ( ! values.get( key ) ) {
				errors.push( `${ file }: missing required ${ key }` );
			}
		}
		const name = values.get( 'name' );
		if ( codexNames.has( name ) ) {
			errors.push( `${ file }: duplicate Codex agent name ${ name }` );
		}
		codexNames.add( name );
	...
	for ( const name of Object.keys( CODEX_AGENTS ) ) {
		if ( ! codexNames.has( name ) ) {
			errors.push(
				`${ codexDirectory }: missing required Codex agent ${ name }`
			);
		}
	}
	...
	if ( settings.model !== 'sonnet' ) {
		errors.push( `${ settingsPath }: expected model "sonnet"` );
	}
	if ( settings.effortLevel !== 'medium' ) {
		errors.push( `${ settingsPath }: expected effortLevel "medium"` );
	}
	...
	for ( const file of claudeFiles ) {
		...
		if ( claudeNames.has( name ) ) {
			errors.push( `${ file }: duplicate Claude agent name ${ name }` );
		}
		claudeNames.add( name );
		const expected = CLAUDE_AGENTS[ name ];
		if ( expected ) {
			if ( fields.get( 'model' ) !== expected.model ) {
				errors.push( `${ file }: expected model ${ expected.model }` );
			}
			if ( fields.get( 'effort' ) !== expected.effort ) {
				errors.push(
					`${ file }: expected effort ${ expected.effort }`
				);
			}
			if (
				fields
				...
				errors.push(
					`${ file }: nested Agent capability is prohibited`
				);
			}
			const tools = fields.get( 'tools' ) ?? '';
			if (
				expected.readOnly &&
				/\b(?:Edit|Write|Bash|Agent)\b/.test( tools )
			) {
				errors.push(
					`${ file }: read-only agent exposes a mutating or nested tool`
				);
			}
			if (
				expected.readOnly &&
				fields.get( 'permissionMode' ) !== 'plan'
			) {
				errors.push(
					`${ file }: read-only agent must use permissionMode plan`
				);
			}
		}
		if ( /^isolation:\s*worktree/m.test( readText( file, errors ) ) ) {
			errors.push( `${ file }: Claude worktree isolation is prohibited` );
		}
	}
	for ( const name of Object.keys( CLAUDE_AGENTS ) ) {
		if ( ! claudeNames.has( name ) ) {
			errors.push(
				`${ claudeDirectory }: missing required Claude agent ${ name }`
			);
		}
	}

	const sharedPolicy = join(
		root,
		'docs',
		'en',
		'04-team-workflow',
	...
	const requiredDocuments = [
		...
		arabicPolicy,
		...rootGuidance,
		join( root, '.codex', 'MODEL-ROUTING.md' ),
		join( root, '.claude', 'MODEL-ROUTING.md' ),
	];
	for ( const file of requiredDocuments ) {
		readText( file, errors );
	}
	const policyText = readText( sharedPolicy, errors );
	for ( const token of [
		'L — low-cost exploration',
		'M — routine work',
		'H — complex/high-risk',
		...
		if ( ! policyText.includes( token ) ) {
			errors.push(
				`${ sharedPolicy }: missing required routing policy section or safeguard: ${ token }`
			);
		}
	}
	for ( const file of rootGuidance ) {
		if ( ! readText( file, errors ).includes( 'Model Routing Gate' ) ) {
			errors.push( `${ file }: must reference Model Routing Gate` );
		}
	}
...
function main() {
	...
	if ( errors.length > 0 ) {
		console.error(
			`Model-routing validation failed with ${ errors.length } issue(s):`
		);
