...
const SITES = [
	{
		order: 1,
		slug: 'bullion-trading-center',
		name: 'Bullion Trading Center',
		url: 'https://bulliontradingcenter.com/',
	},
	{
		order: 2,
		slug: 'etcc',
		name: 'Emirati Talent Competitiveness Council',
		url: 'https://www.etcc.gov.ae/',
	},
	{
		order: 3,
		slug: 'nafis-award',
		name: 'Nafis Award',
		url: 'https://nafisaward.etcc.gov.ae/',
	},
	{
		order: 4,
		slug: 'enrra',
		name: 'Egyptian Nuclear and Radiological Regulatory Authority',
		url: 'https://enrra.org/',
	},
	{
		order: 5,
		slug: 'moe-lessons',
		name: 'Ministry of Education Lessons Platform',
		url: 'https://lessons.moe.gov.eg/',
	},
	{
		order: 6,
		slug: 'oshco',
		name: 'Olayan Saudi Holding Company — OSHCO',
		url: 'https://www.oshco.com/',
	},
	{
		order: 7,
		slug: 'blackstone-eit',
	...
	},
	{
		...
		slug: 'quanta-egypt',
		name: 'Quanta Egypt',
		url: 'https://quanta-egypt.com/',
	},
	{
		order: 9,
		slug: 'hdc-global',
		name: 'HDC Global',
		url: 'https://hdc-global.com/',
	},
	{
		order: 10,
		slug: 'port-said-university',
		name: 'Port Said University',
		url: 'https://psu.edu.eg/',
	},
	{ order: 11, slug: 'rocc', name: 'ROCC', url: 'https://rocc.com/' },
	{
		order: 12,
		slug: 'redcon-properties',
		name: 'Redcon Properties',
		url: 'https://www.redconproperties.com/',
	},
	{
		order: 13,
		slug: 'wis-international',
		name: 'WIS International',
		url: 'https://wisintl.com/',
	},
	{
		order: 14,
		slug: 'eand',
		name: 'e& / Etisalat UAE',
		url: 'https://www.eand.ae/',
	},
	...
	// path; naming it is honest and auditable, and it is two lines rather than a cleverer guess.
	{
		...
		url: 'https://namawomen.ae/',
		logo: 'https://namawomen.ae/wp-content/uploads/sites/9/2024/07/nama-logo-white.png',
	},
	{
		order: 16,
		slug: 'our-forum',
		name: 'Our Forum',
		url: 'https://ourforum.ae/',
		logo: 'https://ourforum.ae/wp-content/uploads/sites/19/2025/04/Dark-LOGO.png',
	},
	{
		order: 17,
		slug: 'cultural-office',
		name: 'Cultural Office',
		url: 'https://culturaloffice.ae/',
	},
	{
		order: 18,
		slug: 'rubu-qarn',
		name: 'Rubu’ Qarn',
		url: 'https://rqsharjah.ae/',
	},
	{
		order: 19,
		slug: 'sharjah-youth',
		name: 'Sharjah Youth',
		url: 'https://shjyouth.ae/',
	},
	{
		order: 20,
		slug: 'reyada-center',
		name: 'Reyada Center',
		url: 'https://reyadacenter.ae/',
	},
	{
		order: 21,
		slug: 'hpd',
	...
	{
		order: 22,
		slug: 'sheikh-sultan-award',
		name: 'Sheikh Sultan Award for Celebrating the Spirit of Youth',
		url: 'https://sheikhsultanaward.ae/',
	},
	{
		order: 23,
		slug: 'children-of-sharjah',
		name: 'Children of Sharjah',
		url: 'https://shjch.ae/',
	},
	{
		order: 24,
		slug: 'sharjah-womens-sports',
		name: 'Sharjah Women’s Sports',
		url: 'https://www.sws.gov.ae/',
	},
	{
		order: 25,
		slug: 'fann-media',
		name: 'FANN Media Discovery Platform',
		url: 'https://fannmedia.ae/',
	},
	{
		order: 26,
		slug: 'child-safety',
		name: 'Child Safety Department',
		url: 'https://childsafety.gov.ae/',
	},
	{
		order: 27,
		slug: 'sharjah-cd',
		name: 'Sharjah Capability Development',
		url: 'https://sharjahcd.ae/',
	},
	{
		order: 28,
		slug: 'sajaya',
	...
	{
		order: 29,
		slug: 'sharjah-olympic-center',
		name: 'Sharjah Olympic Center for Women’s Sports',
		url: 'https://oc.sws.gov.ae/',
	},
];
...
async function get( url, as = 'text' ) {
	...
	/*
	 * The rule guards against work done before an early return wastes it. There is no early return
	 * here, and the ordering is required rather than incidental: the timeout has to be armed before
	 * the fetch it aborts, and `timer` is unconditionally consumed by the `finally` below — which
	 * could not see it if the assignment moved inside the `try`.
	 */
 ...
 * than a mark.
 * @param {string} html the page markup to search
 * @param {string} base the page URL, used to resolve relative sources
 */
function candidates( html, base ) {
	...
	for ( const tag of html.match( /<img\b[^>]*>/gi ) || [] ) {
		const haystack = `${ attr( tag, 'src' ) } ${ attr(
			tag,
			'srcset'
		) } ${ attr( tag, 'alt' ) } ${ attr( tag, 'class' ) } ${ attr(
			tag,
			'id'
		) }`.toLowerCase();
	...
	for ( const tag of links ) {
		if ( attr( tag, 'rel' ).toLowerCase().includes( 'apple-touch-icon' ) ) {
			push( attr( tag, 'href' ) );
		}
	}
	for ( const tag of links ) {
 ...
 * @param {string[]} urls the candidate logo URLs, in discovery order
 * @param {Object}   site the site record whose `slug` supplies the scoring tokens
 */
function rank( urls, site ) {
	const tokens = site.slug
		.split( '-' )
...
function extensionFor( url, bytes ) {
	...
	if (
		[ '.svg', '.png', '.jpg', '.jpeg', '.webp', '.gif' ].includes( fromUrl )
	) {
	...
	if ( bytes.slice( 0, 2 ).toString( 'hex' ) === 'ffd8' ) {
		return '.jpg';
	}
	if (
		bytes.slice( 0, 5 ).toString( 'utf8' ).includes( '<svg' ) ||
		bytes.slice( 0, 200 ).toString( 'utf8' ).includes( '<svg' )
	) {
		return '.svg';
	}
	if ( bytes.slice( 8, 12 ).toString( 'utf8' ) === 'WEBP' ) {
		return '.webp';
	}
	return '';
}
...
async function collect( site, outDir ) {
	...
	if ( urls.length === 0 ) {
		result.note =
			'page fetched, but it exposes no logo, touch icon or og:image';
		return result;
	}
	...
	for ( const url of urls ) {
		try {
			...
			if ( extension !== '.svg' && bytes.length < MIN_BYTES ) {
				continue;
			}

			const file = `${ String( site.order ).padStart( 2, '0' ) }-${
				site.slug
			}${ extension }`;
	...
	result.note =
		result.note || `no usable candidate among ${ urls.length } found`;
	return result;
}
...
for ( const site of SITES ) {
	...
	process.stdout.write(
		`${ result.status === 'ok' ? 'ok  ' : 'MISS' } ${ String(
			site.order
		).padStart( 2 ) }. ${ site.name }` +
			`${
				result.status === 'ok'
					? ` -> ${ result.file } (${ result.bytes }b)`
					: ` -- ${ result.note }`
			}\n`
	);
}
