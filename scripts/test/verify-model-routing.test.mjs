...
import { parseClaudeAgent, validateRepository } from '../verify-model-routing.mjs';
...
test('the validator rejects write-capable critical reviewers', async (t) => {
  const root = await routingFixture();
  t.after(() => rm(root, { recursive: true, force: true }));
  const path = join(root, '.claude', 'agents', 'perego-critical-reviewer.md');
  const text = await readFile(path, 'utf8');
  const parsed = parseClaudeAgent(text, path);
  await writeFile(path, text.replace(parsed.fields.get('tools'), 'Read, Grep, Glob, Write'));
