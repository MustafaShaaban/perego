/**
 * Markup-parity test for the not-found live-canvas block (spec 021 C15).
 *
...
import { NotFoundSkeleton } from './preview';

const fixtureHtml = readFileSync( join( __dirname, '__fixtures__', 'front-not-found.html' ), 'utf8' );

describe( 'not-found markup parity', () => {
	test( 'the editor skeleton matches the PHP front-end markup', () => {
		expect( normalizeMarkup( renderToString( <NotFoundSkeleton /> ) ) )
