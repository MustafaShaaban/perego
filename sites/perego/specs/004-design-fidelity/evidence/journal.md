...
| 6 | Category labels in Arabic | N/A (handoff has no i18n) | (earlier note said AR journal cards showed English category badges) | **Resolved — verified live (2026-07-13)**. The AR `category` terms exist and are translated (`الحرفة` / `ملاحظات الاستوديو` / `الكواليس`), linked to their EN terms via Polylang, and the AR posts are correctly assigned their AR terms — so AR journal cards + the AR single-post badge render Arabic. The earlier "open" note was stale. |
