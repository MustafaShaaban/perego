...
- [~] C12 Journal + Search — **part 1 done:** `journal-header` (editable title/lead per locale via the C11 seed-override seam), `post-breadcrumb` and `post-reading-time` (locked previews), each with a parity test. **Remaining:** `journal-comments`, `related-posts`, `search-results` — all three are context/query-driven (queried post, related query, `?s=`), so they need the dynamic treatment rather than a skeleton: a live search page renders 25 result cards, which is the same 43 KB-fixture problem portfolio-grid had. Plus the `home.html` / `archive.html` T035 fixes.
