...
| Footer social and quick form | Removing legacy block CSS left the old mask-only social spans blank; CoreX's default quick-message labels and lack of placeholders also visibly diverged from the handoff. | The footer now emits the handoff SVG social markup. The supported CoreX schema supplies the reference labels/placeholders while validation and submission storage remain unchanged; required markers are visually hidden only, not removed from the native controls. CV constraint copy and form state captures remain open. |
