...
| 014 | Work archive + project single | `feature/014-work-project-pages` | **DONE** (PR #22) — project pages already CPT-editable (H1←title, meta←spec-010 boxes, body←post-content, grid/nav←Project CPT); added the deferred **gallery wp.media picker**; work archive + project single fidelity verified EN/AR |
| 015 | Journal, single article, search | `feature/015-journal-search-pages` | **DONE** (PR #23) — verification spec: journal archive/single/search are native WP posts + core search via FSE blocks, already handoff-faithful (EN/AR) and natively editable; per-language search + no-results state verified; no code change needed |
| 016 | Contact, forms, form states, email routing | `feature/016-contact-forms-email` | **DONE** (PR #24) — full lifecycle verified with real submissions (states/storage/email); Submissions/Forms&Flows/Data Models functional in wp-admin; fixed two AR i18n gaps (join-form hardcoded placeholders → GlobalContent; missing AR "Select a range"/"Say hello." gettext) |
