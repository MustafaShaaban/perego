...
| 013 | Services archive + four service singles | `feature/013-services-pages-completion` | **DONE** (PR pending) — service-single H1 ← post title (native editing), hero + archive tabs ← editable teaser labels (shared `ServiceCatalog`, per-field seed fallback → byte-identical EN/AR); selected work already ← Projects; fixed archive CTA `?service=` slug bug; whatwedo/process editable via post-content |
