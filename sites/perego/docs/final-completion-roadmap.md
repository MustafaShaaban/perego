...
| 015 | Journal, single article, search | `feature/015-journal-search-pages` | **DONE** (PR pending) — verification spec: journal archive/single/search are native WP posts + core search via FSE blocks, already handoff-faithful (EN/AR) and natively editable; per-language search + no-results state verified; no code change needed |
