...
test( 'a corex block is recognised in the editor inserter', async ( {
	page,
} ) => {
	...
	await expect(
		page.locator( '.block-editor-block-types-list__item' ).first()
	).toBeVisible();
} );

test( 'the contact form validates and accepts a submission', async ( {
	page,
} ) => {
	...
	const message = form
		.locator( 'textarea[name="message"], input[name="message"]' )
		.first();
	...
	await send.click();
	await expect
		.poll( () => name.evaluate( ( el ) => el.checkValidity() ) )
		.toBe( false );
	...
	await send.click();
	await expect(
		page.locator( '.corex-form__status, [role="status"]' ).first()
	).toBeVisible();
} );
