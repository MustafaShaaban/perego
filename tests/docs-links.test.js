...
function offendingUrls( html ) {
	...
	// Code samples are stripped first, because inside one an `href="…"` is *prose about markup*,
	// not a link the browser will follow. The generated class reference is full of them: `DocsUrl`'s
	// docblock explains why a relative `href="/guides/media/"` is wrong in wp-admin, and that
	// sentence renders as a `<code>` element containing those exact characters. Matching it flagged
	// the page that documents the defect as if it contained it.
	//
	// Only visible when the reference pages exist, which is why it went unseen: this test builds
	// whatever `docs-app/dist` happens to hold, and the reference is generated by
	// `composer docs:generate` rather than committed.
	const markup = html
