...
## #140 -- Spec 069: one provider-neutral Model Routing Gate with thin Codex and Claude adapters
Date: 2026-07-12
Context: Perego needs to use OpenAI Codex and Anthropic Claude Code without duplicating workflow rules, spending strong-model capacity on routine work, or pretending that a parent session switched models. The existing Role Gate, Spec Kit, Guard Gate, ownership, no-worktree, and human approval rules must remain authoritative.
Decision: Add one shared Model Routing Gate after Role Gate and before Spec Kit. It defines L (read-only exploration), M (routine work in the balanced parent unless a bounded transfer helps), H (high-risk sole writer), and V (critical read-only review). Codex maps the routes to Luna/Terra/Sol with low/medium/high reasoning and requests Terra/medium as the project default; Claude maps them to Haiku/Sonnet/Opus with low/medium/high effort and requests Sonnet/medium. Codex caps concurrency at three and nesting at one; Claude agents omit the Agent tool. Cross-provider simultaneous writers, Claude worktree isolation, default Ultra/ultracode, duplicate work, and undocumented provider switches are prohibited.
Why: shared route semantics make both providers predictable while the adapters use their current supported configuration surfaces. The balanced parent avoids same-capability delegation overhead; explorers are cheaper read-only workers; strong models are reserved for evidence-based risk; the sole-writer rule avoids merge and ownership conflicts.
