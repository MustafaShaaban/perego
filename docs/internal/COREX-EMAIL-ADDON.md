...
| `AttachmentResolver` (Service) | Maps media-library ids to absolute paths, immediately before send |
...
Attachments are the most dangerous part of an email system, so the rule is short: **a caller names
an attachment by id, never by path.** Nothing in the chain accepts a filesystem path, and that is
the whole of the path-traversal defence — a caller who could pass `../../wp-config.php` would have a
mail-to-self exfiltration primitive.

- **Source:** the WordPress media library, by attachment ID. `AttachmentResolver` maps ids to
  absolute paths internally, immediately before `wp_mail()`.
- **Validation happens where the file is stored, not where it is sent.**
  `Corex\Security\Upload\AttachmentStore` checks the content type against the file's bytes and
  enforces the size cap, so by the time an id exists the file has already been vetted.
- **A missing file is skipped, not fatal.** An attachment that has gone does not stop the message:
  the words are usually the important part, and the file is usually recoverable from the record it
  came from.
...
Mail::to($customer->email)
    ->template('invoice')
    ->with(['order' => $order])
    ->attachMedia($attachmentId)        // from the media library, by id
    ->queue();
```

> **Not implemented: `attach()` and `attachGenerated()`.** Both appeared in this document before the
> capability existed at all — as did `AttachmentResolver`, while `WpMailDriver` called `wp_mail()`
> with four arguments and could not send a file under any circumstances (issue #138, item 9).
> `attach()` implied a path, which is the one thing this design refuses. A framework-generated file
> has no id until something stores it, and once it does, `attachMedia()` is the method you want.
> Spec 081 built what was left and removed the rest of the promise rather than leaving it standing.
